...
public class BroadcastNotifier {
    ...
    public void broadcastIntentWithState(String type, int status, String result) {
        ...
        localIntent.setAction(MyIntentService.BROADCAST_ACTION);

        // Puts the status into the Intent
        localIntent.putExtra(MyIntentService.RESULT_CODE, status);
        localIntent.putExtra(MyIntentService.EXTENDED_OPERATION_TYPE, type);
        if (!result.isEmpty())
    ...
    public void notifyProgress(String logData) {
        ...
        localIntent.putExtra(MyIntentService.EXTENDED_DATA_STATUS, -1);

        // Puts log data into the Intent
        localIntent.putExtra(MyIntentService.EXTENDED_STATUS_LOG, logData);
        localIntent.addCategory(Intent.CATEGORY_DEFAULT);
